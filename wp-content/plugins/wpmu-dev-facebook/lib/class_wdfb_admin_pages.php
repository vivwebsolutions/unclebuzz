...
class Wdfb_AdminPages {
	...
	function register_site_settings () {
		...
		add_settings_field('wdfb_app_key', __('Application ID', 'wdfb'), array($form, create_app_key_box), 'wdfb_options_page', 'wdfb_api');
		add_settings_field('wdfb_api_key', __('API key', 'wdfb'), array($form, create_api_key_box), 'wdfb_options_page', 'wdfb_api');
	...
	function create_blog_admin_menu_entry () {
		...
		//add_options_page('Ultimate Facebook', 'Ultimate Facebook', 'manage_options', 'wdfb', array($this, 'create_admin_page'));
	...
	function create_site_admin_menu_entry () {
				...
				foreach ($keys as $key) {
					if (isset($_POST["wdfb_{$key}"])) {
						...
						if ($override) { // Override child settings
							...
							if (!$blogs) continue;
							foreach ($blogs as $blog) delete_blog_option($blog['blog_id'], "wdfb_{$key}");
						}
					}
				}
				$goback = add_query_arg( 'settings-updated', 'true',  wp_get_referer() );
		...
		//add_submenu_page('settings.php', 'Ultimate Facebook', 'Ultimate Facebook', 'manage_network_options', 'wdfb', array($this, 'create_admin_page'));
	...
	function inject_fb_init_js () {
		echo "<script>
         FB.init({
            appId: '" . $this->data->get_option('wdfb_api', 'app_key') . "', cookie:true,
            status: true,
            cookie: true,
            xfbml: true
         });
	...
	function handle_fb_auth_tokens () {
		...
		//if ($tokens) return true; // We're good

		$fb_uid = $this->model->fb->getUser();

		$app_id = $this->data->get_option('wdfb_api', 'app_key');
		$app_secret = $this->data->get_option('wdfb_api', 'secret_key');
	...
	function publish_post_on_facebook ($id) {
		...
		$post_title = @$_POST['wdfb_metabox_publishing_title'] ? $_POST['wdfb_metabox_publishing_title'] : $post->post_title;
