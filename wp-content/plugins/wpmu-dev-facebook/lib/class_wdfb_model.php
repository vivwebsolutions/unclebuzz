...
class Wdfb_Model {
	...
	function __construct () {
		...
		$this->fb = new Facebook(array(
			...
			'secret' => $this->data->get_option('wdfb_api', 'secret_key'),
			'cookie' => true,
		));
	}
	...
	function get_bp_xprofile_fields () {
		...
		$sql = "SELECT id, name FROM " . $this->db->prefix . "bp_xprofile_fields";
		return $this->db->get_results($sql, ARRAY_A);
	}
	...
	function set_bp_xprofile_field ($field_id, $user_id, $data) {
		...
		$sql = "SELECT id FROM " . $this->db->prefix . "bp_xprofile_data WHERE field_id={$field_id} AND user_id={$user_id}";
		$id = $this->db->get_var($sql);

		if ($id) {
			$sql = "UPDATE " . $this->db->prefix . "bp_xprofile_data SET data='" . $data . "' WHERE id={$id}";
		} else {
			$sql = "INSERT INTO " . $this->db->prefix . "bp_xprofile_data (field_id, user_id, value, last_updated) VALUES (" .
	...
	function set_fb_image_as_bp_avatar ($user_id, $me) {
		...
		// Get FB picture
		$fb_img = file_get_contents("http://graph.facebook.com/{$fb_uid}/picture");
		$filename = md5($fb_uid);
	...
	function get_all_user_tokens () {
		$sql = "SELECT * FROM " . $this->db->prefix . "usermeta WHERE meta_key='wdfb_api_accounts'";
		return $this->db->get_results($sql, ARRAY_A);
	}
	...
	function get_wp_user_from_fb () {
		...
		$sql = "SELECT ID FROM " . $this->db->prefix . "users WHERE user_email=%s";
	...
	function create_new_wp_user_from_fb () {
		...
		$username = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '', $me['email']));
	...
	function get_current_albums () {
		//$fb_user_id = $this->fb->getUser();
		//return $fb_user_id ? $this->get_albums_for($fb_user_id) : false;
