...
class Wdfb_PublicPages {
	...
	function inject_fb_init_js () {
		echo "<script>
         FB.init({
            appId: '" . $this->data->get_option('wdfb_api', 'app_key') . "',
            status: true,
            cookie: true,
            xfbml: true
         });
	...
	/*
	function inject_register_with_facebook_only () {
		if (isset($_GET['fb_register'])) {
			$user_id = $this->model->register_fb_user();
			if ($user_id) {
				//wp_redirect(home_url());
				require_once (WDFB_PLUGIN_BASE_DIR . '/lib/forms/registration_page_success.php');
				exit();
			}
		}
		require_once (WDFB_PLUGIN_BASE_DIR . '/lib/forms/registration_page.php');
		exit();
	}

	function inject_register_with_facebook () {
		if (!isset($_GET['fb_registration_page']) && !isset($_GET['fb_register'])) {
			// Page not requested explicitly, and it's not a save request.
			$url = add_query_arg('fb_registration_page', 1);
			echo '<p><a class="wdfb_register_button" href="' . $url . '">' . __('Register with Facebook', 'wdfb') . '</a></p>';
		} else if (isset($_GET['fb_register'])) {
			// Registration save request
			$user_id = $this->model->register_fb_user();
			if ($user_id) {
				require_once (WDFB_PLUGIN_BASE_DIR . '/lib/forms/registration_page_success.php');
				exit();
			}
		} else if (isset($_GET['fb_registration_page'])) {
	...
	*/
	...
	function inject_fb_comments_admin_og () {
		$app_id = $this->data->get_option('wdfb_api', 'app_key');
		if (!$app_id) return false;
		//echo "<meta property='fb:admins' content='{$app_id}' />\n"; // Deprecated
	...
	function handle_fb_session_state () {
		...
		/*
		// User is not logged in, but we require him to be
		if (!$session && $this->data->get_option('wdfb_connect', 'force_facebook_registration')) {
			wp_logout();
			return;
		}
		*/
			...
			if ($data['registration']) {
				...
				if ($user_id && $wp_grant_blog) {
					...
					if (!$result['errors']->get_error_code()) {
						global $current_site;
						$blog_id = wpmu_create_blog($result['domain'], $result['path'], $result['blog_title'], $user_id, 1, $current_site->id);
	...
	function add_hooks () {
		...
		if ($this->data->get_option('wdfb_connect', 'allow_facebook_registration')) {
			...
			// Jack the signup
			add_action('init', array($this, 'process_facebook_registration'));
/*
			if ($this->data->get_option('wdfb_connect', 'force_facebook_registration')) {
				if (is_multisite()) {
					if (!isset($_GET['fb_registration_page'])) add_action('login_form_register', array($this, 'inject_register_with_facebook_only'));
					else add_action('signup_extra_fields', array($this, 'inject_register_with_facebook_only'));
				} else {
					//die('gtfo');
					add_action('registration_redirect', array($this, 'inject_register_with_facebook_only'));
				}
				add_filter('bp_before_register_page', array($this, 'inject_register_with_facebook_only')); // BuddyPress
				add_action('signup_header', array($this, 'inject_register_with_facebook_only')); // BuddyPress & WPMS
			} else {
				if (is_multisite()) {
					if (!isset($_GET['fb_registration_page'])) add_action('signup_extra_fields', array($this, 'inject_register_with_facebook'));
					else add_action('wp_loaded', array($this, 'inject_register_with_facebook'));
				} else {
					add_action('login_head', create_function('', 'echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"' . WDFB_PLUGIN_URL . '/css/wdfb.css\" />";'));
					if (isset($_GET['action']) && 'register' == $_GET['action'] && !isset($_GET['fb_registration_page']) && !isset($_GET['fb_register'])) add_action('login_message', array($this, 'inject_register_with_facebook'));
					else if (isset($_GET['fb_registration_page']) && !isset($_GET['fb_register'])) add_action('registration_redirect', array($this, 'inject_register_with_facebook'));
					else add_action('registration_redirect', array($this, 'inject_register_with_facebook'));
				}
				add_action('bp_before_register_page', array($this, 'inject_register_with_facebook')); // BuddyPress
			}
*/
		}
